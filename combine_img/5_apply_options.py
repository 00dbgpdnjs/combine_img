...
def browse_dest_path():  # When clicking the "찾아보기" btn / 어디에 저장할지 폴더의 경로 선택하여 그 안에 파일로 저장
    ...
    # When clicking "취소" btn on the 폴더선택 window, folder_seleted will be empty string
    folder_selected = filedialog.askdirectory()
    if folder_selected == '':  # Refer to the code just above
        return  # Exit from browse_dest_path()
...
def merge_image():
    # the value of combobox whose label is "가로넓이"
    ...
    try:
        ...
        if img_width == "원본유지":  # 가로넓이 콤보박스에서 원본유지는 숫자가 아니기 때문에 따로 처리해할 필요가 있다 "원본유지" in the "가로넓이" combobox is not a num, so it needs to be processed separately.
            img_width = -1
        else:
            img_width = int(img_width)  # img_width is str

        img_space = cmb_space.get()
        if img_space == "좁게":
            img_space = 30  # Set space between imgs
        elif img_space == "보통":
            img_space = 60
        elif img_space == "넓게":
            img_space = 90
        ...
        # lower() : Change gotten PNG etc to lowercase
        img_format = cmb_format.get().lower()

        # (Process options / Not Applied options to img)
        # ---------------------------------------
        ...
        # Apply "가로넓이" option : Process one by one by putting size of img in list
        image_sizes = []  # >> [(width1, height1), (width2, height2), ...]
        if img_width > -1:
            # Change width to combo value / Refer to R01
            image_sizes = [(int(img_width), int(
                img_width * x.size[1] / x.size[0])) for x in images]
        else:  # When choosing "원본유지", size[width, height] won't change
            image_sizes = [(x.size[0], x.size[1]) for x in images]

        # R01. How can I 100*60 img to 80width
        # (original width) : (original height) = (changed width) : (changed height)
        #        x         :         y         =        x'       :        y'
        # y' = x'y / x
        # Put the formula above in our code
        # x = size[0]
        # y = size[1]
        # x' = img_width
        ...
        # Difference with the code just above : To apply changed size based on "가로넓이" combo
        ...
        # < Make a sketchbook > : Image.new()
        if img_space > 0:  # Apply space option at the sketchbook's height
        ...
        # !!!This for statement is key codes!!! Merge img with paste() [붙이다]
        for idx, img in enumerate(images):
            # Resize if not "원본유지"
            if img_width > -1:
                img = img.resize(image_sizes[idx])

            # where.paste(what, coordinates on result_img)
            result_img.paste(img, (0, y_offset))
            y_offset += (img.size[1] + img_space)  # height + space
        ...
        file_name = "nado_photo." + img_format  # Apply the format option
        dest_path = os.path.join(txt_dest_path.get(), file_name)
        result_img.save(dest_path)  # Save result_img at dest_path
        msgbox.showinfo("알림", "작업이 완료되었습니다.")
    # except of try...except : Process exception[예외] / Exception : all error occured in merge_image()  to show error msg with showerror() just below / ex. path is c:/ or non-existent cuz window10 can't save in 'c:/' right away
